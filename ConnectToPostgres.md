...
__3a.)__ First, create a ```controllers/``` directory and inside create a ```main.js``` file for the queries & connection:
...
```
__4.)__ Write the db queries inside of the ```main.js``` file: 
...
```
__6.)__ Then inside the main index file, import the queries and setup the routes to call each desired query:
```javascript
// import queries
